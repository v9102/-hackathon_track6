...
class Settings(BaseSettings):
    ...
    def base_dir(self) -> Path:
        ...
        Resolution order:
        1. ``RESUME_AGENT_BASE_DIR`` env var (explicit).
        2. The repository root, detected via the ``pyproject.toml`` marker that
           sits next to ``app/`` (works both in-tree and for editable installs).
        3. The current working directory (pip/non-editable installs, where the
           package is imported from site-packages and should not write there).
        """
        env_override = os.getenv("RESUME_AGENT_BASE_DIR")
        if env_override:
